...
object FunSets {
  ...
  def mapReduce(boundExit: Boolean)(s: Set, p: Int => Boolean) = {
    def iter(a: Int): Boolean = {
      if (a > bound) boundExit
      else if (s(a) && p(a) == !boundExit) !boundExit
      else iter(a + 1)
    }
  ...
  def map(s: Set, f: Int => Int): Set =  e => singletonSet(f(e))(e)
