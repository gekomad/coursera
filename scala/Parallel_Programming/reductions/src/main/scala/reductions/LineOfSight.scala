...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = downsweepSequential(input, output, 0, 0, input.length)
  ...
  def ff(input: Array[Float], i: Int) = input(i) / i
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree =
    if ((end - from) <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val m = (end - from) / 2
      val (tL, tR) = parallel(upsweep(input, from, from + m, threshold), upsweep(input, from + m, end, threshold))
      Node(tL, tR)
    }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    if (from > until) {
      (from to until).foreach(x => output(x) = input(x))
    } else {
      var m = startingAngle
      (from until until).foreach { i =>
        m = max(ff(input, i), m)
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float], threshold: Int): Unit = {
    ...
    output(last) = max(ff(input, last), output(last))
