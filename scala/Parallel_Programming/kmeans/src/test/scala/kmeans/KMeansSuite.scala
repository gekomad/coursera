...
class KMeansSuite extends FunSuite {
  ...
  test("'update classify' should work for 'points' == GenSeq((1, 1, 0), (1, -1, 0), (-1, 1, 0), (-1, -1, 0)) and 'means' == GenSeq((1, 0, 0), (-1, 0, 0))") {
    val p1 = new Point(1, 1, 0)
    val p2 = new Point(1, -1, 0)
    val p3 = new Point(-1, 8, 0)
    val p4 = new Point(-1, -1, 0)
    val points: GenSeq[Point] = IndexedSeq(p1, p2, p3, p4)
    val mean1 = new Point(1, 0, 0)
    val mean2 = new Point(-1, 0, 0)
    val means: GenSeq[Point] = IndexedSeq(mean1, mean2)
    val expected: GenSeq[Point] = GenSeq(new Point(1.0, 0.0, 0.0), new Point(-1.0, 3.5, 0.0))
    val o = classify(points, means)
    val m: GenSeq[Point] = update(o, means)
    assert(m == expected, s"update($o, $means) should equal to $expected")
  }

  ignore("'update until converge") {
    val p1 = new Point(1, 1, 0)
    val p2 = new Point(1, -1, 0)
    val p3 = new Point(-1, 8, 0)
    val p4 = new Point(-1, -1, 0)
    ...
    val means: GenSeq[Point] = IndexedSeq(new Point(1, 0, 0), new Point(-1, 0, 0))

    val eta = 0.001

    val o1 = classify(points, means)
    val newMeans1: GenSeq[Point] = update(o1, means)
    assert(!converged(eta)(means, newMeans1))

    val o2 = classify(points, newMeans1)
    val newMeans2: GenSeq[Point] = update(o2, newMeans1)
    assert(!converged(eta)(newMeans1, newMeans2))

    val o3 = classify(points, newMeans2)
    val newMeans3: GenSeq[Point] = update(o3, newMeans2)
    assert(converged(eta)(newMeans2, newMeans3))
  }
  ...
  test("'converged true'") {
    ...
    val o = converged(0.01)(points, points)
    assert(o == true)
  }
  ...
  test("'converged false'") {
    ...
    val p11 = new Point(1, 1, 0)
    val p21 = new Point(1, -1, 0)
    val p31 = new Point(-1, 80, 0)
    val p41 = new Point(-1, -1, 0)
    val points1: GenSeq[Point] = IndexedSeq(p11, p21, p31, p41)
    val o = converged(0.01)(points, points1)
    assert(o == false)
  }
