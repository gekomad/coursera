 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max of a few numb3ers2") {
    assert(max(List(23, 7, 2)) === 23)
  }

  test("max of a few numbers42") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("max of a few numbers423") {
    intercept[NoSuchElementException] {
      max(Nil)
    }
  }
